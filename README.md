...
[
h_e = \text{GNN}(x_e, G_{line})
]

where:

* (x_e) is the feature vector of road segment (e).
* (G_{line}) is the line graph.
...
* (offset(z_t,e)) is the projected offset along the segment.
* (\theta_e) is the road-segment bearing.

The emission score is:

[
E_\theta(z_t,e) = \text{MLP}*{emission}(r*{t,e})
]
...
\mathcal{L}*{emission}
+
\lambda_T \mathcal{L}*{transition}
+
\lambda_R \lVert \Theta \rVert^2
]
