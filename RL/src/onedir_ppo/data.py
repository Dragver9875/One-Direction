...
def _tensor(value: Any, dtype: torch.dtype | None = None) -> Tensor | None:
    if value is None:
        return None
    out = value if isinstance(value, Tensor) else torch.as_tensor(value)
    return out.to(dtype=dtype) if dtype is not None else out


def _raw_episodes(payload: Any) -> list[Any]:
...
def _episode(raw: Any, fallback_id: int) -> EpisodeSample:
    ...
    candidate_edge_idx = _tensor(raw['candidate_edge_idx'], torch.long)
    candidate_mask = _tensor(raw.get('candidate_mask', candidate_edge_idx >= 0), torch.bool)
    emission_features = _tensor(raw['emission_features'], torch.float32)
    gt_key = 'gt_candidate_pos' if 'gt_candidate_pos' in raw else 'gt_pos'
    gt_candidate_pos = _tensor(raw[gt_key], torch.long).reshape(-1)
    transition_mask = _tensor(raw.get('transition_mask'), torch.bool)
    candidate_proj_xy = _tensor(raw.get('candidate_proj_xy'), torch.float32)
    gt_proj_xy = _tensor(raw.get('gt_proj_xy'), torch.float32)
    gt_edge_idx = _tensor(raw.get('gt_edge_idx'), torch.long)
    ...
    if candidate_edge_idx.ndim != 2 or emission_features.ndim != 3:
        raise ValueError('Invalid episode tensor ranks.')
    return EpisodeSample(trajectory_id, candidate_edge_idx, candidate_mask, emission_features, gt_candidate_pos, transition_mask, candidate_proj_xy, gt_proj_xy, gt_edge_idx, timestamps)
...
def load_rl_dataset(path: str | Path, map_location: str = 'cpu') -> RLDataset:
    ...
    return RLDataset([_episode(raw, i) for i, raw in enumerate(_raw_episodes(payload))])
